...
class particular(torch.nn.Module):
    def __init__(self, D_in, H, D_out):
        ...
        self.a1 = torch.nn.Parameter(torch.Tensor([0.1])).cpu()  # 修改为 CPU
        self.a2 = torch.Tensor([0.1])
        self.a3 = torch.Tensor([0.1])
        self.n = 1 / self.a1.data

        self.fre1 = torch.ones(25).cpu()  # 修改为 CPU
        self.fre2 = torch.ones(25).cpu()  # 修改为 CPU
        self.fre3 = torch.ones(25).cpu()  # 修改为 CPU
        self.fre4 = torch.ones(25).cpu()  # 修改为 CPU
        self.fre5 = torch.ones(25).cpu()  # 修改为 CPU
        self.fre6 = torch.ones(25).cpu()  # 修改为 CPU
        self.fre7 = torch.ones(25).cpu()  # 修改为 CPU
        self.fre8 = torch.ones(25).cpu()  # 修改为 CPU
...
def pred(xy):
    ...
    pred[(xy[:, 1] == 0) & (xy[:, 0] > 0)] = (model_p1(xy[(xy[:, 1] == 0) & (xy[:, 0] > 0)]) + model_p2(
        xy[(xy[:, 1] == 0) & (xy[:, 0] > 0)])) / 2
    return pred
...
model_p1 = particular(2, 30, 1).cpu()
model_p2 = particular(2, 30, 1).cpu()
