...
def interface(Ni):
    ...
    xi = torch.tensor(xi, requires_grad=True)
    return xi
...
def train_data(Nb, Nf):
    ...
    Xf1 = torch.tensor(Xf1, requires_grad=True)
    Xf2 = torch.tensor(Xf2, requires_grad=True)
...
def evaluate(model1, model2):
    ...
    xy_test = torch.from_numpy(xy_test)
...
def run_scale_law(layers, order_num, epoch_num=2500):
    model_p1 = KAN(layers, base_activation=torch.nn.SiLU, grid_size=10, grid_range=[-1.0, 1.0], spline_order=order_num)
    model_p2 = KAN(layers, base_activation=torch.nn.SiLU, grid_size=10, grid_range=[-1.0, 1.0], spline_order=order_num)
    ...
    for epoch in range(epoch_num):
        ...
        def closure():
            ...
            du1dxy = grad(u_pred1, Xf1, torch.ones(Xf1.size()[0], 1), retain_graph=True, create_graph=True)[0]
            du1dx = du1dxy[:, 0].unsqueeze(1)
            du1dy = du1dxy[:, 1].unsqueeze(1)
            du1dxxy = grad(du1dx, Xf1, torch.ones(Xf1.size()[0], 1), retain_graph=True, create_graph=True)[0]
            du1dyxy = grad(du1dy, Xf1, torch.ones(Xf1.size()[0], 1), retain_graph=True, create_graph=True)[0]
            du1dxx = du1dxxy[:, 0].unsqueeze(1)
            du1dyy = du1dyxy[:, 1].unsqueeze(1)

            du2dxy = grad(u_pred2, Xf2, torch.ones(Xf2.size()[0], 1), retain_graph=True, create_graph=True)[0]
            du2dx = du2dxy[:, 0].unsqueeze(1)
            du2dy = du2dxy[:, 1].unsqueeze(1)
            du2dxxy = grad(du2dx, Xf2, torch.ones(Xf2.size()[0], 1), retain_graph=True, create_graph=True)[0]
            du2dyxy = grad(du2dy, Xf2, torch.ones(Xf2.size()[0], 1), retain_graph=True, create_graph=True)[0]
            ...
            du1dxyii = grad(pred_bi1, Xi, torch.ones(Xi.size()[0], 1), retain_graph=True, create_graph=True)[0]
            du2dxyii = grad(pred_bi2, Xi, torch.ones(Xi.size()[0], 1), retain_graph=True, create_graph=True)[0]
